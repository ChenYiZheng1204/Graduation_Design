...
chatComm类：用于连接到Saquib教授的聊天服务器的类
__init__：服务器的IP地址和端口号
startConnection：创建连接到服务器的套接字对象
login：使用用户名和密码向服务器发送登录请求
getFriends：获取与之一起玩游戏的好友列表
sendMessage：向朋友发送消息（游戏信息）
getMail:获取收到的消息（游戏信息）和文件
'''
