...
utils.py description
    is_contious_seq：进行移动并检查是否为连续移动*
    get_move_type：移动并返回其类型和等级*
    convertHelper：接收一个字符串列表并将其转换为列表
    anotherConvertHelper：接收嵌套的字符串列表并将其转换为嵌套列表
    select：接收一些卡片和长度，并返回所需长度的卡片组合列表*
    MovesGenerat类：用于生成可能的移动的类*
    注：以上以*结尾的模块表示它们的灵感来源于DouZerohttps://github.com/kwai/DouZero
'''
